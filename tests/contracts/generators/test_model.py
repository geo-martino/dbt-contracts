...
from dbt_contracts.contracts.generators.model import ModelGenerator
from tests.contracts.generators.test_node import NodeGeneratorTester


class TestModelGenerator(NodeGeneratorTester[ModelNode]):
    @pytest.fixture
    ...
    def test_generate_table_patch(self, generator: ModelGenerator, item: ModelNode):
        table = generator._generate_table_patch(item)
        assert all(val for val in table.values())

    def test_generate_new_patch(self, generator: ModelGenerator, item: ModelNode):
    ...
    def test_update_existing_patch_with_empty_patch(
            self, generator: ModelGenerator, item: ModelNode, context: ContractContext
    ):
    ...
    def test_update_existing_patch_with_new_table(
            self, generator: ModelGenerator, item: ModelNode, models: list[ModelNode], context: ContractContext
    ):
        key = item.resource_type.pluralize()
